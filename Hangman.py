...
# Getting valid word only from words
...
# Function to provide hints to the user when certain criteria meets
def hint(word_letter):
    return word_letter.pop()


def hangman():
    # Getting the word from words and putting it into the set
    ...
    while len(word_letters) > 0 and lives > 0:
        # Joining the user entered letters
        print('You have', lives, ' lives. You have used these letters: ', ' '.join(used_letters))

        # Giving the word view and hints
        word_list = [letter if letter in used_letters else '-' for letter in word]
        print('Current Word: ', ' '.join(word_list))
        if lives == 6:
            print('Hint: ', hint(word_letters))
        if lives == 3:
            print('Hint: ', hint(word_letters))

        # Checking if the user given alphabet is there in word or not
        ...
        if user_letter in alphabet - used_letters:
            ...
            # If user input doesn't match the letter in word, lives will decrease by one
            else:
                lives -= 1
                print("Letter is not in word.")

        # If you try to re-enter the letter which is already disqualified, you will be prompted with this message
    ...
    # If lives == 0 then the game will terminate
    if lives == 0:
